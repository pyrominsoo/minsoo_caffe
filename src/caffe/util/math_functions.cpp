...
void minsoo_sgemm_bfloat16(const CBLAS_TRANSPOSE TransA, const CBLAS_TRANSPOSE TransB, 
    ...
    for (int row = 0; row < M; row++) {
        for (int col = 0; col < N; col++) {
            ...
            for (int k_index = 0; k_index < K; k_index++) {
				float tempA = 0;
				float2bfloat(op_A[row][k_index], tempA);
				float tempB = 0;
				float2bfloat(op_B[k_index][col], tempB);
				float product = tempA * tempB;
                float b_product = 0;
                float2bfloat(product, b_product);
                accum = accum + b_product;
            }
...
void minsoo_sgemv_bfloat16(const CBLAS_TRANSPOSE TransA, 
    ...
    if (TransA == CblasTrans) {
        ...
        for (int row = 0; row < N; row++) {
            ...
            for (int col = 0; col < M; col++) {
                float tempA = 0;
                float2bfloat(A[col*N+row], tempA);
                float tempB = 0;
                float2bfloat(x[col], tempB);
                float product = tempA * tempB;
                float b_product = 0;
                float2bfloat(product, b_product);
                accum = accum + b_product;
        ...
        for (int row = 0; row < M; row++) {
            ...
            for (int col = 0; col < N; col++) {
                float tempA = 0;
                float2bfloat(A[row*N+col], tempA);
                float tempB = 0;
                float2bfloat(x[col], tempB);
                float product = tempA * tempB;
                float b_product = 0;
  ...
  // cblas_sgemm(CblasRowMajor, TransA, TransB, M, N, K, alpha, A, lda, B,
    ...
    switch (mult_type) {
        ...
        case 11 :
            ...
            break;
        case 12 : 
            minsoo_sgemm_bfloat16(TransA, TransB, M, N, K, alpha, A, B, beta, C);
            break;
        default :
...
void minsoo_gemv_special1<float>(const CBLAS_TRANSPOSE TransA, const int M,
    ...
    switch (mult_type) {
        ...
        case 11 :
            ...
            break;
        case 12 : 
            minsoo_sgemv_bfloat16(TransA, M, N, alpha, A, x, beta, y); 
            break;
        default :
            std::cout << "undefined mult_type: " << mult_type << std::endl;
            exit(1);
