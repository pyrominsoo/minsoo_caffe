...
void minsoo_sgemm_mitchell(const CBLAS_TRANSPOSE TransA, const CBLAS_TRANSPOSE TransB, 
    ...
    static int count_gemm = 0;
    std::ofstream fout;
    fout.open("gemm.log",ios::out | ios::app);
    if (gemm_report) {
        count_gemm++;
        fout << "\n\n";
        fout << "Beginning the output of GEMM at Number " << count_gemm << "\n";
    }
    ...
    for (int row = 0; row < M; row++) {
        for (int col = 0; col < N; col++) {
            ...
            for (int k_index = 0; k_index < K; k_index++) {
                ...
                if (gemm_report) {
                    fout << "A : " << temp << "\n";
                    fout << "B : " << op_B[k_index][col] << "\n";
                }
                mitch_mult(&temp, &op_B[k_index][col],64);
