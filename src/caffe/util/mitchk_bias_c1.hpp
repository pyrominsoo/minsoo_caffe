#ifndef MITCHK_BIAS_C1_HPP_
#define MITCHK_BIAS_C1_HPP_
...
void mitchk_bias_c1(Fixed<I,F> *op1, Fixed<I,F> *op2, unsigned int k)
{
    Fixed<I,F> orig1;
    Fixed<I,F> orig2;
    Fixed<128,0> N1;
    ...
    if (N1 == 0) {
        ...
        if (op1_log_int >= k) {
            ...
            N1 = temp;
            // Round up
            trunc_amt = k - op1_log_int;
            N1 <<= trunc_amt;
            N1 += 1;
            N1 <<= (total_bits - 1 - op1_log_int) - trunc_amt;
        }
#ifdef MDEBUG
    ...
    if (N2 == 0) {
        ...
        if (op2_log_int >= k) {
            ...
            N2 = temp;
            // Round up
            trunc_amt = k - op2_log_int;
            N2 <<= trunc_amt;
            N2 += 1;
            N2 <<= (total_bits - 1 - op2_log_int) - trunc_amt;
        }
...
#ifdef MDEBUG
    std::cout << "added_log before bias: " << std::fixed << added_log << "\n";
#endif
